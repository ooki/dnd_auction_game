...
class ClusterBot:
    ...
    def k_means(self, points):
        ...
        for _ in range(10):  # Iterations
            ...
            for point in points:
                distances = [
                    self.calculate_distance(point, centroid)
            ...
            for i, cluster in enumerate(clusters):
                if cluster:
                    self.centroids[i] = [sum(d) / len(cluster) for d in zip(*cluster)]

        return clusters

    def bid(
        self, agent_id, current_round, states, auctions, prev_auctions, reminder_info
    ):
        ...
        high_value_cluster = max(
            clusters, key=lambda c: sum(f[0] * (f[1] + 1) / 2 + f[2] for f in c)
        )
        bids = {}
